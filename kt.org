#+begin_src latex
  ...
  $v$. The failure link of $v$ is found by visiting its parent's failure
  link, $w=f(\mbox{parent}(v))$. If that has a child $w\rightarrow w'$
  with $\mbox{in}(w')=\mbox{in}(v)$, the failure link of $v$ is set to
  $w'$. In our example, the failure link of node 3 with path label
  \texttt{AT} is node 7 with path label \texttt{T}. If no match is
...
#+begin_src latex
  ...
  Failure links are constructed during a breadth-first traversal of the
