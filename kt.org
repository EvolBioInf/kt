#+begin_src latex
   ...
   $v$. The failure links of the root and its children are initialized
   to the root. By default, the failure link of any other node, $v$, is
   ...
   label \texttt{AT} is node 7 with path label \texttt{T}. If no match
   is found, the procedure is repeated until the root is reached. If no
   failure link is found at all, use the root. Figure~\ref{fig:kt2}
   shows our example tree with failure links.
...
#+end_src
#+begin_src latex
  The function \texttt{BreadthFirst} takes as arguments the root
  and a function applied to every node. This function might in turn have
